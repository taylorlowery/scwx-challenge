...
@pytest.mark.reverse_word
def test_reverse_word_empty_string():
    LOG.info("test_reverse_word_empty_string()")
    expected = ""
    actual = reverse_string("")
    assert expected == actual

@pytest.mark.reverse_word
def test_reverse_word_escaped_characters():
    LOG.info("test_reverse_word_escaped_characters()")
    expected = "\f\b\t\r\n\"\\\'"
    actual = reverse_string("\'\\\"\n\r\t\b\f")
    assert expected == actual

@pytest.mark.reverse_word
def test_reverse_word_escaped_hex_values():
    LOG.info("test_reverse_word_escaped_hex_values()")
    expected = "\x31\x32\x33\x34"
    actual = reverse_string("\x34\x33\x32\x31")
    assert expected == actual

@pytest.mark.reverse_word
def test_reverse_word_escaped_ansi_color_code():
    LOG.info("test_reverse_word_escaped_ansi_color_code()")
    expected = "\033[1,37m"
...
def test_reverse_word_other_languages(input):
    LOG.info(f"test_reverse_word_other_languages(\"{ input }\")")
...
def test_reverse_word_other_data_types_raise_value_error(input):
    LOG.info(f"test_reverse_word_other_data_types_raise_value_error({ input })")
